...
namespace MongoDBPrototype
{
    public class MongoDBWrapper
    {
        ...
        public async Task<List<BsonDocument>> LogicalAnd(List<Tuple<string, string>> fieldValuePairs)
        {
            ...
            filter = fieldValuePairs.Skip(1)
                .Aggregate(filter, (current, pair) => current & builder.Eq(pair.Item1, pair.Item2));
            return await collection.Find(filter).ToListAsync();
        }
        ...
        public async Task<string> CreateACompoundIndex()
        {
            ...
            IndexKeysDefinition<BsonDocument> keys =
                Builders<BsonDocument>.IndexKeys.Ascending("cuisine").Ascending("address.zipcode");
            return await collection.Indexes.CreateOneAsync(keys);
        }
