...
namespace MongoDBPrototype.Tests
{
    public class MongoDBWrapperTests
    {
        private MongoDBWrapper _mongoTest;
        ...
        {
            await _mongoTest.DropRestaurantsCollection();
            _mongoTest.KillServerAndCleanup();
        }
        ...
        [Test]
        public async void ShouldBeAbleToFindOutIfACollectionExists()
        {
            Assert.That(await MongoDBWrapper.CollectionExists("restaurants"));
        }

        [Test]
        public async void GetCollectionShouldNotCreateTheCollection()
        {
            IMongoCollection<BsonDocument> mongoCollection = MongoDBWrapper.GetCollection<BsonDocument>("badCollectionName");
            Assert.That(await MongoDBWrapper.CollectionExists("badCollectionName"), Is.False);
        }

        [Test]
        public async void ShouldBeAbleToCreateACollection()
        {
            await _mongoTest.CreateCollection("newCollection");
            Assert.That(await MongoDBWrapper.CollectionExists("newCollection"));
            await _mongoTest.DropCollection("newCollection");
        }
    }
}